...
# -----------------------------------------------------------------------------
# Expression evaluator / reflection pass
# -----------------------------------------------------------------------------


def _build_expression_evaluator_query(
    *,
    state: AdvisorState,
    ...
    """
    Build a second-pass evaluator prompt.

    This does NOT generate SQL.
    It only checks whether the compiled ATTRIBUTE_VALUE expression fully captures
    the user request and runtime-spec rules.
    """
    plan = state.get("plan", {}) or {}
...
def _call_expression_evaluator_llm(evaluator_query: str) -> str:
    try:
        from openai import OpenAI
    except ModuleNotFoundError as exc:
        raise RuntimeError(
...
def _parse_expression_evaluator_answer(raw_answer: str) -> dict[str, Any]:
    text = _strip_json_fences(raw_answer)
...
def _evaluator_decision_to_structured_answer(
    *,
    original_structured_answer: dict[str, Any],
    evaluator_decision: dict[str, Any],
) -> dict[str, Any]:
    """
    Convert evaluator decision back into the same structured schema used by the
    compiler. Then _parse_structured_answer validates it again.
    """
    ...
    if decision == "reject":
        return _parse_structured_answer(
            {
                "evaluator": RULEBOOK_EVALUATOR,
...
def _final_expression_safety_issues(
    *,
    operation_type: str,
    operation_kind: str,
    expression: str,
    ...
    has_source_field_signal = _text_has_any_signal(combined_text, SOURCE_FIELD_SIGNALS)
    has_unit_conversion_signal = _text_has_any_signal(combined_text, UNIT_CONVERSION_SIGNALS)

    has_boolean_conversion_signal = _text_has_any_signal(
        combined_text,
        [
            "boolean",
            "bool",
...
def _evaluate_compiled_expression(
    *,
    state: AdvisorState,
    structured_answer: dict[str, Any],
) -> tuple[dict[str, Any], str, list[str], list[str]]:
    """
    Run an LLM evaluator/reflection pass over the compiled expression.

    Returns:
    - possibly corrected structured_answer
    - raw evaluator answer
    - warnings
    - errors
    """
    plan = state.get("plan", {}) or {}
    ...
    for attempt in range(MAX_EXPRESSION_EVALUATOR_RETRIES + 1):
        evaluator_query = _build_expression_evaluator_query(
            state=state,
        ...
        try:
            raw_evaluator_answer = _call_expression_evaluator_llm(evaluator_query)
            evaluator_decision = _parse_expression_evaluator_answer(raw_evaluator_answer)
            evaluated_answer = _evaluator_decision_to_structured_answer(
                original_structured_answer=current_answer,
                evaluator_decision=evaluator_decision,
            )
        except Exception as exc:
            # Do not fail the whole node just because evaluator failed.
            # The original compiler output still goes through hard Python safety checks.
        ...
        safety_issues = _final_expression_safety_issues(
            operation_type=operation_type,
            operation_kind=evaluated_answer["operation_kind"],
            expression=evaluated_answer["expression"],
            rhs_request=rhs_request,
...
def dsl_expression_node(state: AdvisorState) -> dict[str, Any]:
    ...
    # Second-pass evaluator / reflection check.
    # This catches cases where the first compiler found only one meaning
    # but the user requested multiple transformations, e.g.
    # "dto field flags.isActive and convert it to BOOLEAN".
